...
1. Call `get_reasoning_context` with:
...
4. Call `check_plan` with the same task/files plus your `plan`.
...
- If the same test or command fails twice with the same error, call `rescue_failure` with the error text. Do not run that command a third time first.
- Never store hidden reasoning anywhere. The runtime stores only observable facts.

## After finishing

1. Call `record_trace` with the observable summary:
   - `agent`: `"codex"`
...
2. For high-risk domains (`beseam.shopify.publish`, `beseam.pdp.schema`, `beseam.catalog.fix`, `beseam.tracker.classification`) also call `run_rubric_gate` with the rubric id for the domain.
...
The server exposes: `get_reasoning_context`, `check_plan`, `rescue_failure`, `record_trace`, `extract_reasonblock`, `run_rubric_gate`.
