...
- `make verify` passes (ruff, black --check, mypy --strict, pytest).
- `make verify-local` passes.
- `make verify-mcp` passes.
- `make phase-t-hardening` passes and writes a report under `.atelier/reports/phase_t/`.
