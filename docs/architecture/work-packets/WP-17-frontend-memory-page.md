...
- `frontend/src/services/stub/...` — regenerated by `make generate-stub` (no manual edits)
- `frontend/src/pages/Traces.tsx` — edit (add a "Open run inspector" button)

## How to execute

1. Run `make generate-stub` first so the new memory endpoints from WP-07/08 are in the SDK.
