...
export default function Blocks() {
  ...
  const [filter, setFilter] = useState<"all" | "active" | "retired" | "deprecated">("all");
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[10px] px-1.5 py-0.5 font-bold uppercase tracking-tight font-mono ${map[status] || map.retired}`}>
      {status}
    </span>
  );
}
...
function BlockDetail({ block }: { block: ReasonBlock }) {
  ...
  const successRate = total > 0 ? Math.round((block.success_count / total) * 100) : null;
  ...
  return (
    <div className="space-y-5 text-sm">
      ...
      <header className="pb-4 border-b border-neutral-800">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          ...
          {block.task_types.map((t) => (
            <span key={t} className="text-[10px] px-1.5 py-0.5 bg-neutral-900 border border-neutral-700 text-neutral-400 font-mono">
              {t}
            </span>
          ))}
        </div>
        <h2 className="text-base font-bold text-neutral-300 leading-snug">{block.title}</h2>
        <div className="font-mono text-[10px] text-neutral-600 mt-1">{block.id}</div>
        <div className="flex gap-4 mt-2 text-[10px] text-neutral-600">
          <span>Created {new Date(block.created_at).toLocaleString()}</span>
          {block.updated_at && <span>· Updated {new Date(block.updated_at).toLocaleString()}</span>}
        </div>
        {/* Stats */}
        {total > 0 && (
          <div className="flex gap-3 mt-3">
            <Stat label="Uses" value={total} />
            <Stat label="✓ Success" value={block.success_count} color="text-emerald-400" />
            <Stat label="✗ Failures" value={block.failure_count} color="text-red-400" />
            {successRate !== null && <Stat label="Rate" value={`${successRate}%`} color={successRate >= 70 ? "text-emerald-400" : "text-amber-400"} />}
          </div>
        )}
      </header>
        ...
        <section>
          ...
          <ol className="space-y-2">
            {block.procedure.map((step, i) => (
              <li key={i} className="flex gap-3 bg-neutral-900/40 border border-neutral-800  px-3 py-2.5">
        ...
        <section>
          ...
          <ul className="space-y-1.5">
            {block.verification.map((v, i) => (
              <li key={i} className="flex gap-2 items-start text-[13px] text-emerald-300 bg-emerald-950/20 border border-emerald-900/30 px-3 py-2">
        ...
        <section>
          ...
          <ul className="space-y-1.5">
            {block.dead_ends.map((d, i) => (
              <li key={i} className="flex gap-2 items-start text-[13px] text-red-300 bg-red-950/20 border border-red-900/30 px-3 py-2">
        ...
        <section>
          ...
          <ul className="space-y-1.5">
            {block.failure_signals.map((s, i) => (
              <li key={i} className="flex gap-2 items-start text-[13px] text-amber-300 bg-amber-950/20 border border-amber-900/30 px-3 py-2">
      ...
      {(block.triggers.length > 0 || block.file_patterns.length > 0 || block.tool_patterns.length > 0) && (
        <MatchHints block={block} />
      )}
    </div>
  );
}

function Stat({ label, value, color = "text-neutral-300" }: { label: string; value: string | number; color?: string }) {
  return (
      ...
      {open && (
        <div className="space-y-2">
          {block.triggers.length > 0 && (
            <ChipRow label="Triggers" items={block.triggers} color="bg-blue-950/40 text-blue-300 border-blue-900/40" />
          )}
          {block.file_patterns.length > 0 && (
            <ChipRow label="File patterns" items={block.file_patterns} color="bg-purple-950/40 text-purple-300 border-purple-900/40" mono />
          )}
          {block.tool_patterns.length > 0 && (
            <ChipRow label="Tool patterns" items={block.tool_patterns} color="bg-neutral-800 text-neutral-300 border-neutral-700" mono />
          )}
        </div>
      )}
    </section>
...
function ChipRow({ label, items, color, mono = false }: { label: string; items: string[]; color: string; mono?: boolean }) {
  return (
    <div>
      <div className="text-[9px] uppercase text-neutral-600 mb-1">{label}</div>
      <div className="flex flex-wrap gap-1.5">
        {items.map((item) => (
          <span key={item} className={`text-[11px] px-2 py-0.5 border ${color} ${mono ? "font-mono" : ""}`}>
            {item}
          </span>
        ))}
      </div>
